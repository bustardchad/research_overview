...
for ds in ts:
 #       time = ds.current_time.in_units("Kyrs")
        dd = ds.all_data()
        hot_ad = dd.cut_region(['obj["gas", "temperature"] > 2e3'])
 ...
 #       print(ds.derived_field_list)
        ...
        def _rotationMeasureAbs(field, data):
            return np.abs(data['rotationMeasure'])

        ds.add_field(('gas','rotationMeasureAbs'), function=_rotationMeasure, units="gauss/cm**3", take_log=True, display_name='|Rotation Measure|', sampling_type="cell")
       ...
       # d = yt.ProjectionPlot(ds, "z", "rotationMeasure",fontsize = 20)
        L = [-0.561,0.099,0.822] # vector normal to cutting plane
        d = yt.OffAxisProjectionPlot(ds, L, "rotationMeasure",center=[0.0,0.0,0.0], width=(10, 'kpc'),fontsize=20,data_source = hot_ad)
       ...
       # d = yt.ProjectionPlot(ds, "z", "density")
        d.set_cmap(field="rotationMeasure", cmap='PuOr')
        d.set_colorbar_label("rotationMeasure", "$\phi$ (rad $ m^{-2}$)")
        #d.set_width((60,'kpc'))
       # d.set_zlim('rotationMeasure',-200,200)
        ...
        plot = yt.PhasePlot(dd, ("gas", "nenh_interp"), ("gas", "temperature"), [("gas", "cell_mass")], weight_field=None,fractional=True)
        plot.set_xlim(1e-3, 1.2)
        plot.set_ylim(1000, 1e5)
        plot.save()

        plot = yt.PhasePlot(dd, ("gas", "nenh_interp"), ("gas", "temperature"), [("gas", "rotationMeasureAbs")], weight_field=None,fractional=True)
        plot.set_xlim(1e-3, 1.2)
        plot.set_ylim(1000, 1e5)
        plot.set_zlim(("gas", "rotationMeasureAbs"),1e-4, 5e-1)
        plot.save()

        """

        prj_fits = yt.FITSOffAxisProjection(ds,L,'rotationMeasure',center = (0,0,0),width=(10, 'kpc'), data_source = hot_ad)
       # sky_center = [82.245,-69.5] # in degrees
        ...
        prj_fits.writeto("LMC_Wind_Ram_evolved_Faraday_above2e3K.fits", clobber=True)
        # prj_fits.write_fits("LMC_ram.fits", clobber=True,sky_center=sky_center,sky_scale=sky_scale)
        image_data = fits.getdata('LMC_Wind_Ram_evolved_Faraday_above2e3K.fits')
        print(type(image_data))
        print(image_data.shape)

        fig = aplpy.FITSFigure("LMC_Wind_Ram_evolved_Faraday_above2e3K.fits")
        fig.add_grid()
       # fig.show_colorscale(cmap="PuOr",stretch = 'linear',vmin=-200, vmax = 200)
        fig.show_colorscale(cmap="PuOr",stretch = 'linear',vmin=-200, vmax = 200)
       ...
       # fig.tick_labels.set_xformat('dd')
        ...
        fig.savefig('Wind_Ram_evolved_above2e3K.pdf')
        """
