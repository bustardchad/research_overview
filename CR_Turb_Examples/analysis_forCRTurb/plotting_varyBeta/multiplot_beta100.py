...
for i in times:
 ...
 fn5 = "../beta100/"   # beta = 10
 base = 'cr.out1.'
 ds5 = yt.load(fn5+base+str(i).zfill(5)+'.athdf')

 fn6 = "../beta100/"
 base = 'cr.out2.'
 ds6 = yt.load(fn6+base+str(i).zfill(5)+'.athdf')
 ...
 ds5.add_field(('gas', u'pcr'), function = _pcr, units="g/(cm*s**2)", dimensions=dimensions.pressure)
 ds5.add_field(('gas', u'densratio'), function = _densratio, units="",display_name=r"$\rho / \rho_{i}$", dimensions=dimensions.density)
 ds6.add_field(("gas","crscale"), function=crscale,display_name=r"$L_{CR}/L_{0}$",units="")
 
 grad_fields = ds.add_gradient_fields(("gas","pcr"))
 grad_fields3 = ds3.add_gradient_fields(("gas","pcr"))
 grad_fields5 = ds5.add_gradient_fields(("gas","pcr"))
 ...
 for j in range(3):
   ...
   if (j==0):
     ...
     slc = yt.SlicePlot(ds5, 'z', plotvar,fontsize=fsize)
     slc.set_zlim(plotvar, varmin, varmax)
     slc.set_xlabel('x (kpc)')
     slc.set_ylabel('y (kpc)')
     slc.hide_axes()
     ...
     slc.set_log(plotvar,False)
     slc.annotate_line_integral_convolution(("gas", "magnetic_field_x"), ("gas", "magnetic_field_y"), kernellen = 100, lim=(0.5, 0.65),alpha = 0.6)
     slc.annotate_title(r"$\beta \sim 100$")
     plot = slc.plots[plotvar]
     plot.figure = fig
   ...
   if (j==2):
     ...
     slc = yt.SlicePlot(ds6, 'z', plotvar,fontsize=fsize)
     slc.set_zlim(plotvar, varmin, varmax)
     slc.set_cmap(field=plotvar, cmap='hot_r')
     slc.set_xlabel('x (kpc)')
     slc.set_ylabel('y (kpc)')
 ...
 plt.savefig('multiplot_beta1_beta10_beta50_'+str(i).zfill(5)+'.pdf')
