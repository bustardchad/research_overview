...
num_procs = 1
fns = glob.glob('../Stream_hdf5_plt_cnt_*')
fns.sort()
...
j = 0
for i in times:
    ds = yt.load('../Stream_hdf5_plt_cnt_'+str(i).zfill(4))
#for sto,fn in yt.parallel_objects(fns,num_procs,storage=data):
   # ds = yt.load(fn)
    ...
    frb = proj.to_frb(width=(100,'kpc'),resolution=[512,512])
 ...
 #   print(filtered_image)
    ...
    data[j].append([time, maxCol, avgCol, stdCol, total])
    j = j+1
...
file1 = open("CoolFile_512.txt","w")#write mode
#if yt.is_root()
...
for val in range(0,20):
    file1.write("     %05.3f      %5.3g mR     %5.3g mR     %5.3g mR         %5.3e      \n" % \
